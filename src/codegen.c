...
static void write_strbuf_to_file_path(const char* path, darr(yap_strbuf) bufs){
	FILE* f = fopen(path, "w");
	if (!f){
		yap_log("Failed to open output file: %s", path);
		return;
	}
	for_darr(i, buf, bufs){
		if (buf.data && buf.len > 0){
			fwrite(yap_strbuf_data(&buf), 1, buf.len, f);
			fputc('\n', f);
		}
	}
	fclose(f);
	yap_log("C code saved to %s", path);
}

static int yap_c_emit_files(yap_module* module, const char* out_dir){
	if (!module || !module->module_ctx || !out_dir) return 0;
	...
	char types_path[YAP_PATH_MAX];
	char proto_path[YAP_PATH_MAX];
	char impl_path[YAP_PATH_MAX];
	snprintf(types_path, sizeof(types_path), "%s/types.h", out_dir);
	snprintf(proto_path, sizeof(proto_path), "%s/prototypes.h", out_dir);
	snprintf(impl_path, sizeof(impl_path), "%s/impl.c", out_dir);

	write_strbuf_to_file_path(types_path, mod_code->types);
	write_strbuf_to_file_path(proto_path, mod_code->decls);

	// impl.c gets the preambule
	FILE* f = fopen(impl_path, "w");
	if (f){
		fwrite(preambule, 1, sizeof(preambule)-1, f);
		fputs("#include \"types.h\"\n#include \"prototypes.h\"\n\n", f);
		for_darr(i, buf, mod_code->impl){
			if (buf.data && buf.len > 0){
				fwrite(yap_strbuf_data(&buf), 1, buf.len, f);
				fputc('\n', f);
			}
		...
		fclose(f);
		yap_log("C code saved to %s", impl_path);
	}

	// Compile with gcc, output binary to CWD
	char cmd[YAP_PATH_MAX * 2];
	snprintf(cmd, sizeof(cmd), "gcc %s/impl.c -o a.out 2>&1", out_dir);
	yap_log("Compiling: %s", cmd);
	int ret = system(cmd);
	if (ret != 0) {
		yap_log("GCC COMPILATION FAILED (exit code %d). Run: gcc %s/impl.c -o a.out", ret, out_dir);
	} else {
...
yap_ctx* yap_emit(yap_ctx* ctx){
	...
	if (out_dir){
		yap_log("Temp build dir: %s", out_dir);
		int gcc_ret = yap_c_emit_files(mod, out_dir);
		free(out_dir);
		if (gcc_ret != 0) {
			yap_emit_error_no_pos(ctx, "GCC compilation failed (exit code %d)", gcc_ret);
		}
	} else {
		yap_log("Failed to create temp build directory");
	}
