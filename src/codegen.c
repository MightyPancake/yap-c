...
static void yap_c_emit_files(yap_module* module, const char* out_dir){
	if (!module || !module->module_ctx || !out_dir) return;
	...
	int ret = system(cmd);
	if (ret != 0)
		yap_log("Compilation failed with exit code %d", ret);
	else
